...
int match(uint64_t a, uint64_t b) {
    int m = 0;
    a ^= b;
    for (int i = 0; i < 64; i++) {
        if ((a >> i) & 1) {
...
int main(int argc, char** argv) {
    ...
    vector<vector<vector<vector<int>>>> results(8, vector<vector<vector<int>>>(8, vector<vector<int>>(height, vector<int>(width, 0))));

    for (int offsetY = 0; offsetY < 8; offsetY++) {
        for (int offsetX = 0; offsetX < 8; offsetX++) {

            vector<vector<uint64_t>> canvas(height, vector<uint64_t>(width, 0));
            for (int row = 0; row < height; row++) {
                for (int col = 0; col < width; col++) {
                    uint64_t tile = 0;
                    for (int i = 0; i < 8; i++) {
                        for (int j = 0; j < 8; j++) {
                            Color pixel = colorArray[(row * 8 + i + offsetY) * image.width + col * 8 + j + offsetX];
                            if (pixel.r > 125) {
                                tile |= 1;
                            }
                            tile <<= 1;
            ...
            for (int i = 0; i < height; i++) {
                for (int j = 0; j < width; j++) {
                    ...
                    for (int k = 0; k < NUMCHARS; k++) {
                        int newMatch = match(canvas[i][j], fontInts[k]);
                        if (newMatch > highest) {
                            bestMatch = k;
                            highest = newMatch;
                        }
                    }
                    results[offsetY][offsetX][i][j] = fontChars[bestMatch];
                }
            }
    ...
    while (!WindowShouldClose()) {
        ...
        DrawTexture(original, 0, 0, WHITE);
        for (int offsetX = 0; offsetX < 8; offsetX++) {
            for (int offsetY = 0; offsetY < 8; offsetY++) {
                for (int i = 0; i < height; i++) {
                    for (int j = 0; j < height; j++) {
                        DrawTextEx(font, TextToUtf8(&(results[offsetY][offsetX][i][j]), 1), 
                            {image.width + 9 * width * offsetX + 8 * j, 9 * width * offsetY + 8 * i}, 8, 0, BLACK);
                    }
                }
            }
        }
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            ...
            int xSelect = (mouse.x - image.width) / (width * 9 * 8);
            int ySelect = mouse.y / (height * 9 * 8);
            if (xSelect >= 0 && xSelect < 8 && ySelect >= 0 && ySelect < 8) {
                ofstream out;
                out.open(argv[2]);
                for (int i = 0; i < height; i++) {
                    for (int j = 0; j < width; j++) {
                        out << TextToUtf8(&(results[ySelect][xSelect][i][j]), 1);
                    }
                    out << endl;
                }
                out.close();
                ...
                cout << "Bad selection." << xSelect << " " << ySelect << endl;
            }
        }
        EndDrawing();
    }
